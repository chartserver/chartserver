/*
Copyright 2019 Replicated, Inc..

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	v1beta1 "github.com/charthq/chartserver/pkg/apis/chartserver/v1beta1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeChartVersions implements ChartVersionInterface
type FakeChartVersions struct {
	Fake *FakeChartserverV1beta1
	ns   string
}

var chartversionsResource = schema.GroupVersionResource{Group: "chartserver.io", Version: "v1beta1", Resource: "chartversions"}

var chartversionsKind = schema.GroupVersionKind{Group: "chartserver.io", Version: "v1beta1", Kind: "ChartVersion"}

// Get takes name of the chartVersion, and returns the corresponding chartVersion object, and an error if there is any.
func (c *FakeChartVersions) Get(name string, options v1.GetOptions) (result *v1beta1.ChartVersion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(chartversionsResource, c.ns, name), &v1beta1.ChartVersion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1beta1.ChartVersion), err
}

// List takes label and field selectors, and returns the list of ChartVersions that match those selectors.
func (c *FakeChartVersions) List(opts v1.ListOptions) (result *v1beta1.ChartVersionList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(chartversionsResource, chartversionsKind, c.ns, opts), &v1beta1.ChartVersionList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1beta1.ChartVersionList{ListMeta: obj.(*v1beta1.ChartVersionList).ListMeta}
	for _, item := range obj.(*v1beta1.ChartVersionList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested chartVersions.
func (c *FakeChartVersions) Watch(opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(chartversionsResource, c.ns, opts))

}

// Create takes the representation of a chartVersion and creates it.  Returns the server's representation of the chartVersion, and an error, if there is any.
func (c *FakeChartVersions) Create(chartVersion *v1beta1.ChartVersion) (result *v1beta1.ChartVersion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(chartversionsResource, c.ns, chartVersion), &v1beta1.ChartVersion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1beta1.ChartVersion), err
}

// Update takes the representation of a chartVersion and updates it. Returns the server's representation of the chartVersion, and an error, if there is any.
func (c *FakeChartVersions) Update(chartVersion *v1beta1.ChartVersion) (result *v1beta1.ChartVersion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(chartversionsResource, c.ns, chartVersion), &v1beta1.ChartVersion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1beta1.ChartVersion), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeChartVersions) UpdateStatus(chartVersion *v1beta1.ChartVersion) (*v1beta1.ChartVersion, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(chartversionsResource, "status", c.ns, chartVersion), &v1beta1.ChartVersion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1beta1.ChartVersion), err
}

// Delete takes name of the chartVersion and deletes it. Returns an error if one occurs.
func (c *FakeChartVersions) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(chartversionsResource, c.ns, name), &v1beta1.ChartVersion{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeChartVersions) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(chartversionsResource, c.ns, listOptions)

	_, err := c.Fake.Invokes(action, &v1beta1.ChartVersionList{})
	return err
}

// Patch applies the patch and returns the patched chartVersion.
func (c *FakeChartVersions) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1beta1.ChartVersion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(chartversionsResource, c.ns, name, pt, data, subresources...), &v1beta1.ChartVersion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1beta1.ChartVersion), err
}
